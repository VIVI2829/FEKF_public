...
class ExtendedKalmanFilter:
    def __init__(self,model, dt, std = 1.5):
        ...
        self.mea_beacon = 4
        self.motion_cov = torch.eye(4, dtype=torch.float64)
        self.measurement_cov = torch.sqrt(self.std) * torch.eye(self.mea_beacon, dtype=torch.float64)
...
model = torch.load('trained_model/v10newNVP_4Aff_3hiddlayer_64hid_size8x8_10x10samples_std15_1000000.pth', map_location=torch.device('cpu')).double()
# model = torch.load('trained_model/v6newNVP_4Aff_4hidd_size8x8_10samples_std15_100000.pth', map_location=torch.device('cpu'))
# trained_model/v6newNVP_4Aff_4hidd_size8x8_10samples_std15_100000.pth
model.double()
model.eval()
std = 1.5
traj_values = 3
samples = 200  # Số lượng mẫu khởi tạo
trials = 200  # Mặc định số lần thử nghiệm Monte Carlo
...
for traj in range(traj_values):
    ...
    for _ in range(trials):
        ...
        # if NF_mse > 10:
        #     continue
        NF_mse_trials.append(NF_mse.detach().numpy())
        Vanilla_mse_trials.append(Vanilla_mse.detach().numpy())

    del RSS, pos_vanilla, Vanilla_mse, pos, NF_mse
    NF_mse_mean = np.mean(NF_mse_trials)
    Vanilla_mse_mean = np.mean(Vanilla_mse_trials)

    print("NF mean_mse", NF_mse_mean)
    print("Vanilla mean_mse", Vanilla_mse_mean)

    NF_mse_median = np.median(NF_mse_trials)
    Vanilla_mse_median = np.median(Vanilla_mse_trials)

    print("NF median_mse", NF_mse_median)
    print("Vanilla median_mse", Vanilla_mse_median)

    NF_mse_results.append(NF_mse_trials)
    Vanilla_mse_result.append(Vanilla_mse_trials)


NF_mse_results = pd.DataFrame(NF_mse_results)
Vanilla_mse_result = pd.DataFrame(Vanilla_mse_result)
# NF_results = pd.DataFrame(NF_mse_results, index=range(traj_values))
NF_mse_results.to_csv("v10Monte_Carlo_EKF_NF_mse.csv", header=False)
# Vanilla_results = pd.DataFrame(NF_mse_results, index=range(traj_values))
Vanilla_mse_result.to_csv("v10Monte_Carlo_EKF_Vanilla_mse.csv", header=False)
#
