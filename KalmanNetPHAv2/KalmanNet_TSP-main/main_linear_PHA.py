...
[train_input, train_target, cv_input, cv_target, test_input, test_target] = torch.load('INPUT/22012012/cal_Net_y.pt', map_location=device)
...
folderName = 'KNet/2201/'
model_name = '2201model_y.pt'
# KNet_model = KalmanNetNN()

KNet_model = torch.load(folderName + model_name)
KNet_model.eval()
...
KNet_Pipeline = Pipeline(strTime, folderName, model_name)
...
if (KnownRandInit_test): 
   ...
   ## Test Neural Network
   print("Compute Loss on All States (if false, loss on position only):", Loss_On_AllState)
   [MSE_test_linear_arr, MSE_test_linear_avg, MSE_test_dB_avg,KNet_out,RunTime] = KNet_Pipeline.NNTest(sys_model, test_input, test_target, folderName,MaskOnState=not Loss_On_AllState,randomInit=True,test_init=test_init)
else: 
   print("Test KNet with Unknown Initial State")
   ## Test Neural Network
   print("Compute Loss on All States (if false, loss on position only):", Loss_On_AllState)
   [MSE_test_linear_arr, MSE_test_linear_avg, MSE_test_dB_avg,KNet_out,RunTime] = KNet_Pipeline.NNTest(sys_model, test_input, test_target, folderName,MaskOnState=not Loss_On_AllState)
...
print(len(df_KNet))
# df_KNet.to_csv("OUTPUT/KNET/2904/fulldata2904ypredictedbyKNETy2201.csv", index=False)
# df_KNet.to_csv("OUTPUT/KNET/2201/fulldata2201predictedbyKNETy2904.csv", index=False)
#vdf_KNet.to_csv("OUTPUT/KNET/2201/samedayKNETy2201.csv", index=False)
# df_KNet.to_csv("OUTPUT/KNET/2201/samedayKNETy2201.csv", index=False)
